...
5. **Google Python Style Guide + ruff 규칙**:
   ...
   - line-length 80
6. **한글 주석 / docstring 의무화** — 네가 작성하는 모든 자연어 텍스트(클래스/메서드 docstring, 인라인 주석, 로그 메시지 등)는 **반드시 한글**로 써라. 사람이 유지보수할 때 언어 혼용이 부채가 되는 것을 방지한다. 불필요한 주석을 추가하라는 뜻이 아니라, **쓸 때는 한글** 이라는 의미 — CLAUDE.md 의 "WHY 가 비자명할 때만 주석" 원칙은 그대로 유지. 함수/변수/클래스 **식별자 자체는 영어** 로 (파이썬 관례 + ruff N 룰).
7. **PostToolUse `ruff-format` hook** 이 저장 시 자동으로 돌고 수정 불가 에러가 남으면 차단. 차단되면 에러를 읽고 고친 뒤 재시도.
