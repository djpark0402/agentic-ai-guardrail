...
def test_playground_defaults_dev_returns_values() -> None:
    ...
    with patch("app.config.get_settings") as mock_settings:
        ...
        mock_settings.return_value.playground_default_api_key = "uak_demo"
        mock_settings.return_value.playground_default_hmac_secret = SecretStr(
            "s3cret"
        )
...
def test_playground_defaults_unset_returns_empty() -> None:
    ...
    with patch("app.config.get_settings") as mock_settings:
        mock_settings.return_value.app_env = "dev"
        mock_settings.return_value.playground_default_api_key = ""
        mock_settings.return_value.playground_default_hmac_secret = SecretStr(
            ""
        )
        response = client.get("/v1/playground/defaults")
